...
function Filters({filters,setFilters,options,open,setOpen}){
  ...
  const emptyLabels={project:"All projects",location:"All locations",department:"All departments",designation:"All designations",category:"All categories",state:"All workforce states"};
  return <aside className={`card manpower-filters ${open?"open":""}`} aria-label="Manpower filters">
    ...
    {FILTER_KEYS.map(key=><SmartSelect key={key} label={labels[key]} value={filters[key]}
      options={[{value:"",label:emptyLabels[key]},...options[key]]} onChange={value=>update(key,value)} className="manpower-select"/>)}
...
function ScenarioPanel({selected,projects,state,setState,target,setTarget,onApply,onClose}){
  return <section className="card scenario-builder" aria-label="Temporary allocation scenario">
    ...
    <SmartSelect label="Proposed state" value={state} options={Object.entries(STATE_LABELS).map(([value,label])=>({value,label}))}
      onChange={setState} className="manpower-select"/>
    {state==="allocated"&&<SmartSelect label="Target project" value={target}
      options={projects.map(project=>({value:project.code,label:`${project.code} · ${project.name}`}))}
      onChange={setTarget} className="manpower-select"/>}
  ...
  return <div className="manpower-workspace">
    ...
    <div className="manpower-layout"><div className="manpower-content">
      ...
      <section className="manpower-chart-grid">
        <ChartCard title="Employees by current project"><ResponsiveContainer width="100%" height={chartHeight}><BarChart data={projectData} layout="vertical" barCategoryGap="35%" margin={{left:8,right:22}}><CartesianGrid strokeDasharray="3 3" horizontal={false}/><XAxis type="number" allowDecimals={false}/><YAxis dataKey="name" type="category" width={115} tick={{fontSize:10}}/><Tooltip/><Bar dataKey="count" maxBarSize={30} fill="var(--chart-primary)" radius={[0,5,5,0]} onClick={entry=>{const name=entry?.payload?.name||entry?.name;const project=projects.find(item=>item.name===name);setChartFilter("project",project?.code||"")}}/></BarChart></ResponsiveContainer></ChartCard>
        ...
        <ChartCard title="Employees by current location"><ResponsiveContainer width="100%" height={chartHeight}><BarChart data={locationData} barCategoryGap="35%" margin={{left:0,right:8}}><CartesianGrid strokeDasharray="3 3" vertical={false}/><XAxis dataKey="name" tick={{fontSize:9}} interval={0}/><YAxis allowDecimals={false}/><Tooltip/><Bar dataKey="count" maxBarSize={30} fill="var(--chart-primary)" radius={[5,5,0,0]} onClick={entry=>setChartFilter("location",entry?.payload?.name||entry?.name)}/></BarChart></ResponsiveContainer></ChartCard>
        <ChartCard title="Current project by category"><ResponsiveContainer width="100%" height={chartHeight}><BarChart data={categoryData.data} barCategoryGap="35%"><CartesianGrid strokeDasharray="3 3" vertical={false}/><XAxis dataKey="name" tick={{fontSize:9}} interval={0}/><YAxis allowDecimals={false}/><Tooltip/><Legend/>{categoryData.categories.map((category,index)=><Bar dataKey={category} maxBarSize={30} stackId="category" fill={CATEGORY_COLORS[index]} key={category} onClick={entry=>setChartFilter("category",category)}/>)}</BarChart></ResponsiveContainer></ChartCard>
        <ChartCard title="Top designations"><ResponsiveContainer width="100%" height={chartHeight}><BarChart data={designationData} layout="vertical" barCategoryGap="35%" margin={{left:8,right:22}}><CartesianGrid strokeDasharray="3 3" horizontal={false}/><XAxis type="number" allowDecimals={false}/><YAxis dataKey="name" type="category" width={120} tick={{fontSize:10}}/><Tooltip/><Bar dataKey="count" maxBarSize={30} fill="#8b5cf6" radius={[0,5,5,0]} onClick={entry=>setChartFilter("designation",entry?.payload?.name||entry?.name)}/></BarChart></ResponsiveContainer></ChartCard>
        <ChartCard title="Leave balance band"><ResponsiveContainer width="100%" height={chartHeight}><BarChart data={leaveData} barCategoryGap="35%"><CartesianGrid strokeDasharray="3 3" vertical={false}/><XAxis dataKey="name" tick={{fontSize:9}} interval={0}/><YAxis allowDecimals={false}/><Tooltip/><Bar dataKey="count" maxBarSize={30} fill="#22a447" radius={[5,5,0,0]}/></BarChart></ResponsiveContainer></ChartCard>
      </section>
