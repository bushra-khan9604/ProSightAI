...
try{
 ...
 await reset('cancel');await ask();await page.getByRole('button',{name:'Cancel',exact:true}).waitFor();mode='normal';await page.reload();await page.getByText(result.answer,{exact:true}).waitFor();assert.equal(posts,1);console.log('PASS refresh resumes without new query');
 ...
 await fs.mkdir('outputs/ui-refinements',{recursive:true});await page.screenshot({path:'outputs/ui-refinements/light.png'});
